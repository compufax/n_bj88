...
function RowHeader($data)
{
	...
	for($i=0;$i<count($data);$i++)
	{
		//Ponemos el color de relleno a usar
		$this->SetFillColor(209,226,239);	
		...
		$this->Rect($x,$y,$w,$h,'F');
		//Print the text
		$this->MultiCell($w,5,$data[$i],1,'C',$a);
...
function RowHeaderGetColor($data,$color)
{
	//Calculate the height of the row
	$nb=0;
	for($i=0;$i<count($data);$i++)
...
function RowGetColor($data,$color)
{
	//Calculate the height of the row
	$nb=0;
	for($i=0;$i<count($data);$i++)
...
/*******clase agregada por ERHV para imprimir textos con etiquetas  <bold >  [cell] ***/
