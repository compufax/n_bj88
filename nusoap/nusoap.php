...
class nusoap_base {
	...
	* @access   private
	*/
	function debug($string, $ponerlo = false){
		if ($this->debugLevel > 0 && $ponerlo) {
...
//		}
	...
	*/
	function sendRequest($data, $cookies = NULL) {
		...
		// build payload
		$this->debug('Data antes de buildpayload: '.$data);
		$this->buildPayload($data, $cookie_str);
		$this->debug('bulidpayload: '.$this->outgoing_payload, 1);
	  if ($this->io_method() == 'socket') {
		// send payload
...
class nusoap_client extends nusoap_base  {
	...
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
		...
		switch(true){
			...
			case preg_match('/^http/',$this->endpoint):
				...
				$this->debug('sending message, length='.strlen($msg).', '.$msg);
				...
				if($err = $http->getError()){
					...
					$this->debug('got response, length='. strlen($this->responseData).' '.$this->responseData.' type='.$http->incoming_headers['content-type']);
					return $this->parseResponse($http->incoming_headers, $this->responseData);
				}
			break;
			default:
				$this->setError('no transport found, or selected transport is not yet supported!'.$this->endpoint);
			return false;
			break;
		}
	}
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->debug('Use encoding: ' . $this->xml_encoding . ' when creating nusoap_parser');
		$this->debug('Datos de parse: '.$data);
